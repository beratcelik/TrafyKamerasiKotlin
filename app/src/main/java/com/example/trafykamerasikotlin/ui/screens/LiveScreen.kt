...
fun LiveScreen(
    ...
    LaunchedEffect(deviceIp) {
        if (deviceIp != null) viewModel.startStream(deviceIp)
    }
    ...
    Box(
        ...
        when (val state = uiState) {
            ...
            is LiveUiState.StoppingRecording -> StoppingPlaceholder()
            is LiveUiState.Playing           -> RtspPlayer(rtspUrl = state.rtspUrl)
        }
    }
}
...
@Composable
private fun StoppingPlaceholder() {
    Column(
        horizontalAlignment = Alignment.CenterHorizontally,
        verticalArrangement = Arrangement.spacedBy(16.dp)
    ) {
 ...
 *
 * ExoPlayer, Android MediaPlayer, and libVLC all fail against HiDVR
 * dashcam RTSP because they abort when the camera returns 400 to the
 * RTSP OPTIONS request.  IjkPlayer uses FFmpeg's libavformat RTSP
 * client which ignores the OPTIONS failure and continues to DESCRIBE,
 * exactly as the reference GoLook app does.
 *
 * Key FFmpeg options:
