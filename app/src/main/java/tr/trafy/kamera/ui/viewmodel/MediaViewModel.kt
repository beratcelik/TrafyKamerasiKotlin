...
import tr.trafy.kamera.data.media.AllwinnerMediaRepository
import tr.trafy.kamera.data.media.AllwinnerSdInfo
import tr.trafy.kamera.data.media.CamHttpGate
import tr.trafy.kamera.data.media.EeasytechMediaRepository
import tr.trafy.kamera.data.media.GeneralplusMediaRepository
import tr.trafy.kamera.data.media.HiDvrMediaRepository
import tr.trafy.kamera.data.model.ChipsetProtocol
import tr.trafy.kamera.data.model.DeviceInfo
import tr.trafy.kamera.data.model.MediaFile
import tr.trafy.kamera.data.network.DashcamHttpClient
import tr.trafy.kamera.data.settings.AiOverlayPreferences
...
class MediaViewModel(app: Application) : AndroidViewModel(app) {
    ...
    private val mstarRepo     = tr.trafy.kamera.data.media.MstarMediaRepository()
     ...
     * downloaded this clip yet. The Play action prefers local playback
     * over a fresh cam stream when a downloaded copy exists.
     */
    ...
    fun startAllwinnerStream(file: MediaFile) {
        ...
        allwinnerStreamJob = viewModelScope.launch {
            ...
            val tsServer = tr.trafy.kamera.data.allwinner.LiveTsHttpServer()
     ...
     * [tr.trafy.kamera.data.allwinner.MpegTsMuxer] and pushed
    ...
    private suspend fun streamAllwinnerFile(
        ...
        val muxer = tr.trafy.kamera.data.allwinner.MpegTsMuxer(tsOut)
        var videoCount = 0
        var audioCount = 0
        var audioBytes = 0L
        var audioFrames = 0
            ...
            coroutineScope {
                ...
                try {
                    client.packets().collect { sample ->
                        ...
                        when (sample) {
                            is tr.trafy.kamera.data.allwinner.AllwinnerRtp2pClient.Sample.Video -> {
                                videoCount++
                                val aus = tr.trafy.kamera.data.allwinner
                            ...
                            is tr.trafy.kamera.data.allwinner.AllwinnerRtp2pClient.Sample.Audio -> {
                                audioCount++
                                audioBytes += payload.size
                                val frames = tr.trafy.kamera.data.allwinner
