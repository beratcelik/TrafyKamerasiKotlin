...
class EeasytechSettingsRepository(private val context: Context) {
    ...
    suspend fun exitSettings(deviceIp: String) {
        if (tr.trafy.kamera.data.media.EeasytechCleanupLock.ranRecently()) {
            Log.d(TAG, "exitSettings: skipped (sibling cleanup ran recently)")
            return
        }
        tr.trafy.kamera.data.media.EeasytechCleanupLock.mutex.withLock {
            if (tr.trafy.kamera.data.media.EeasytechCleanupLock.ranRecently()) {
                Log.d(TAG, "exitSettings: skipped inside lock")
                return@withLock
            }
            ...
            tr.trafy.kamera.data.media.EeasytechCleanupLock.markCompletion()
        }
    }
